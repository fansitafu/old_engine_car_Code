...
#include "ifr_crc.h"
/* Private variables -------------------------------------------------*/
static const uint8_t IFR_CRC8_Table[256] = //CRC8 多项式：x8+x5+x4+1
{
...
static const uint16_t IFR_CRC16_Table[256] = //CRC16 多项式：x16+x12+x5+1
{
...
/*
	...
	*	@reval：crc8的值。
*/
uint8_t IFR_Get_CRC8_Check(uint8_t *pchMessage, uint32_t DataLength, uint8_t crc8_init) //解析
{
	uint8_t ucIndex;
...
uint16_t IFR_Get_CRC16_Check(uint8_t *pchMessage, uint32_t DataLength, uint16_t crc16_init)
{
	...
	{
		chData = *pchMessage++;
		(wCRC) = ((uint16_t)(wCRC) >> 8) ^ IFR_CRC16_Table[((uint16_t)(wCRC) ^ (uint16_t)(chData)) & 0x00ff];
	}
	return wCRC;
}
/**
  * @概述	CRC16验证函数
  * @参数1	p_msg：待验证的完整数据包（含数据+末尾2字节CRC校验值）
  * @参数2	len：完整数据包长度（数据长度 + 2字节CRC）
  * @返回值 1：验证通过；0：验证失败/参数无效
  */
uint8_t verify_crc16_check_sum(uint8_t *p_msg, uint16_t len)
{
    uint16_t crc_calculated = 0; // 计算出的CRC16值
    uint16_t crc_received = 0;   // 数据包中接收的CRC16值
    // 参数有效性检查：空指针/长度不足（至少需包含2字节CRC）
    if((p_msg == NULL) || (len <= 2))
        return 0;
    crc_calculated = IFR_Get_CRC16_Check(p_msg, len - 2, 0xFFFF);
    crc_received = (uint16_t)p_msg[len - 2] | ((uint16_t)p_msg[len - 1] << 8);
