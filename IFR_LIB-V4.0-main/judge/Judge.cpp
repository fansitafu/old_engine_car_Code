...
void JudgeData_analysis(uint8_t *pata, uint8_t len)
{
	...
	while(i < len - 5)
	{
		if(IFR_Get_CRC8_Check(&pata[i], 4, IFR_CRC8_INIT) == pata[i + 4])
		{
			count_tem_1++;
			...
			if(IFR_Get_CRC16_Check(&pata[i], 5 + 2 + data_length, IFR_CRC16_INIT) == CRC16)
			{
				count_tem_2++;
				Judge_Data_TF = 1;
				switch(Cmd_ID)
				{
					...
					case GAME_State				: memcpy(&my_game_status,					&pata[i+7],	data_length); break;// = 0x0001,	//比赛状态数据  memcpymemcpy        11 Byte   1HZ发送
					case Competition_result_data				: memcpy(&my_game_result_t,					&pata[i+7],	data_length); break;// = 0x0002,//比赛结果数据          1 Byte   比赛结束触发发送
					case Robot_HP_data				: memcpy(&my_game_robot_HP_t,					&pata[i+7],	data_length); break;// = 0x0003,//机器人血量数据，         32Byte   固定以3Hz频率发送 
					case Event_data				: memcpy(&my_event_data,					&pata[i+7],	data_length); break;// = 0x0101,	//场地事件数据     		4 Byte   事件改变后发送 
					...
					case RobotStateId   : memcpy(&my_robot_status,					&pata[i+7],	data_length);break;//		= 	0x0201,	//机器人性能体系数据  	13 Byte   10Hz 
					...
					case BuffMuskId	: memcpy(&my_buff,					&pata[i+7],	data_length);break;//			= 	0x0204,	//机器人增益和底盘能量数据				7  Byte		固定以3Hz频率发送 

					case Damage_status  : memcpy(&my_robot_hurt,					&pata[i+7],	data_length);break;// 	=   0x0206,	//伤害状态数据			    1  Byte		伤害发生后发送 
					case Real_shooting   : memcpy(&my_shoot_data,					&pata[i+7],	data_length);break;//	=   0x0207,	//实时射击数据			    7  Byte	  弹丸发射后发送 
					case Allowable_firing  : memcpy(&my_projectile_allowance_t,					&pata[i+7],	data_length);break;//=   0x0208,	//允许发弹量			      6  Byte	  固定以10Hz频率发送 
